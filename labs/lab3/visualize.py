...
targets = [
    transform( np.array([-.1, -.32, .53]), np.array([0,pi,pi])          ),
    transform( np.array([-.22, .4, .5]),  np.array([pi/6,5/6*pi,7/6*pi]) ),
    transform( np.array([.53, 0, .45]),    np.array([0,pi,pi])            ),
    transform( np.array([.7, -.15, .4]),    np.array([0,pi,pi])            ),
    transform( np.array([.35, .64, 0.4]),  np.array([0,pi,pi])            ),
]
...
if __name__ == "__main__":
    ...
    for i, target in enumerate(targets):
        ...
        start = perf_counter()
        q, rollout, success, message = ik.inverse(target, seed, method='J_trans', alpha=.5)  #try both methods
        stop = perf_counter()
        dt = stop - start
