...
class JacobianDemo():
    ...
    def eight(t,fx=0.5,fy=1.0,rx=.15,ry=.1):
        """
        Calculate the position and velocity of the figure 8 trajectory
        ...
        Rdes = np.diag([1., -1., -1.])
        ang_vdes = np.array([0.0, 0.0, 0.0])

        return Rdes, ang_vdes, xdes, vdes

    def ellipse(t, f=0.5, rx=.15, ry=.10):
        """
        Calculate the position and velocity of the ellipse trajectory
        ...
        x0 = np.array([0.307, 0, 0.487]) # corresponds to neutral position
        xdes = x0 + np.array([rx * sin(f * t), ry * cos(f * t), 0])
        vdes = np.array([rx * f * cos(f * t), -ry * f * sin(f * t), 0])
        Rdes = np.diag([1., -1., -1.])
        ang_vdes = np.array([0.0, 0.0, 0.0])
    ...
    def line(t, f=1.0, L=.15):
        """
        Calculate the position and velocity of the line trajectory
        ...
        # Calculate desired position and velocity for line
